<x-admin.app-layout>
    ...
    <x-admin.breadcrumb>
        <x-slot:breadcrumb_title>
            <h3>Add New {{ trans('cruds.bankOfficer.title_singular') }} {{ trans('global.list') }}</h3>
        </x-slot:breadcrumb_title>
        <li class="breadcrumb-item"><a href="{{ route('admin.bank-officers.index') }}">{{ trans('cruds.bankOfficer.title_singular') }}</a></li>
        <li class="breadcrumb-item active">Add New {{ trans('cruds.bankOfficer.title_singular') }} {{ trans('global.list') }}</li>
    </x-admin.breadcrumb>
    <div class="container-fluid">
        <div class="card">
            <div class="card-body p-3">
                <form method="post" action="{{ route('admin.bank-officers.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        ...
                        <div class="form-group col-12 col-md-6">
                            ...
                            <input type="text" class="form-control" id="ic" name="ic" value="" required/>
                        </div>
{{--                        <div class="form-group col-12 col-md-6">--}}
                        ...
                        <div class="form-group col-12 col-md-4">
                            ...
                            <select class="form-control select_search {{ $errors->has('banks') ? 'is-invalid' : '' }}"
                                ...
                                @foreach($banks as $id => $bank)
                                    <option value="{{ $id }}"{{ old('banks') === (string) $id ? 'selected' : '' }}>{{ $bank }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-12 col-md-4">
                            <label class="required" for="bank_account">Bank Account</label>
                            <input type="text" class="form-control" id="bank_account" name="bank_account" value="" required/>
                        </div>
                        <div class="form-group col-12 col-md-4">
                            <label class="required" for="commission">Commission (%)</label>
                            <input type="number" class="form-control" id="commission" name="commission" value="" min="0.0" step="0.1" required/>
                        </div>
                    </div>
