...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.userTeam.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-teams.update", [$userTeam->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.userTeam.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $userTeam->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('team_lead') ? 'is-invalid' : '' }}" name="team_lead_id" id="team_lead_id">
                    @foreach($team_leads as $id => $entry)
                        <option value="{{ $id }}" {{ (old('team_lead_id') ? old('team_lead_id') : $userTeam->team_lead->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('team_members') ? 'is-invalid' : '' }}" name="team_members[]" id="team_members" multiple>
                    @foreach($team_members as $id => $team_member)
                        <option value="{{ $id }}" {{ (in_array($id, old('team_members', [])) || $userTeam->team_members->contains($id)) ? 'selected' : '' }}>{{ $team_member }}</option>
                    @endforeach
                </select>
