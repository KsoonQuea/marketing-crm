    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.criteria.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="name">{{ trans('cruds.criterion.fields.name') }}</label>
            ...
            <div class="form-group">
                <label>{{ trans('cruds.criterion.fields.status') }}</label>
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    <option value disabled {{ old('status', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Criterion::STATUS_SELECT as $key => $label)
