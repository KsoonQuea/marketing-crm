...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.caseCreditCheck.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.case-credit-checks.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('credit_check') ? 'is-invalid' : '' }}" name="credit_check_id" id="credit_check_id">
                    @foreach($credit_checks as $id => $entry)
                        <option value="{{ $id }}" {{ old('credit_check_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('finding') ? 'is-invalid' : '' }}" type="text" name="finding" id="finding" value="{{ old('finding', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('migration') ? 'is-invalid' : '' }}" type="text" name="migration" id="migration" value="{{ old('migration', '') }}">
