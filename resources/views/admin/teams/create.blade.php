<x-admin.app-layout>
    ...
    <x-admin.breadcrumb>
        <x-slot:breadcrumb_title>
            <h3>{{ trans('global.create') }} {{ trans('cruds.team.title_singular') }}</h3>
        </x-slot:breadcrumb_title>
        <li class="breadcrumb-item"><a href="{{ route('admin.teams.index') }}">{{ trans('cruds.team.title_singular') }}
                {{ trans('global.list') }}</a></li>
        <li class="breadcrumb-item active">{{ trans('global.create') }} {{ trans('cruds.team.title_singular') }}</li>
    </x-admin.breadcrumb>
            ...
            <div class="col-12 col-md-6 col-lg-5">
                <div class="card">
                    <div class="card-body p-3 pb-0">
                        <form method="POST" action="{{ route("admin.teams.store") }}" enctype="multipart/form-data" class="form theme-form">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <div class="w-full h-full overflow-auto">
                                    <input type="number" class="form-control pull-left" name="commission_percent" value="{{ old('commission_percent',0.00) }}" min="0.00" step="0.01" max="100.00" style="width:80px;"/>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2 {{ $errors->has('leadername') ? 'is-invalid' : '' }}" name="leadername" id="leadername">
                                    <option value disabled {{ old('leadername',null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                                    @foreach($sales_manager as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ old('leadername') === (string) $id ? 'selected' : '' }}>{{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2 {{ $errors->has('member') ? 'is-invalid' : '' }}" name="member[]" id="member" multiple>
                                    @foreach($users as $id => $entry)
                                        <option value="{{ $id }}" {{ old('member') === (string) $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
