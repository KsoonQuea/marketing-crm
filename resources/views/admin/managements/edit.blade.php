            ...
            <div class="col-12 col-md-6 col-lg-5">
                <div class="card">
                    <div class="card-body p-3">
                        <form method="POST" action="{{ route("admin.management.update",[$managements->id]) }}" enctype="multipart/form-data">
                            @method('PUT')
                            @csrf
                            <div class="form-group">
                                ...
                                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                                    @foreach($users as $id => $entry)
                                        <option value="{{ $id }}" {{ ($managements->user_id == $id) ? 'selected' : ''  }}>{{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <div class="w-full h-full overflow-auto">
                                    <input type="number" class="form-control pull-left" name="commission_rate" value="{{ old('commission_rate',$managements->commission_rate) }}" min="0.00" step="0.01" max="100.00" style="width:80px;"/>
