<x-admin.app-layout>
    ...
    <x-admin.breadcrumb>
        <x-slot:breadcrumb_title>
            <h3>{{ trans('global.edit') }} {{ trans('cruds.user.title_singular') }}</h3>
        </x-slot:breadcrumb_title>
        <li class="breadcrumb-item">
            <a href="{{ route('admin.users.index') }}">{{ trans('cruds.user.title_singular') }} {{ trans('global.list') }}</a>
        </li>
...
{{--                        </div>--}}
                        ...
                        <div class="form-group col-12 col-md-3 {{ $errors->has('roles') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2 {{ $errors->has('roles') ? 'is-invalid' : '' }}"
                                    name="roles" id="roles" onchange="showBfeInfo()">
                                    <option value disabled {{ old('roles',null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                                @foreach($roles as $id => $role)
                                    <option value="{{ $id }}" @selected(old('roles', $user->roles?->first()?->id) == $id)>{{ $role }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('roles'))
                        ...
                        <div class="form-group col-12 col-md-3 {{ $errors->has('roles') ? 'has-error' : '' }}" id="class_div" style="display: none">
                            ...
                            <select class="form-control select2 {{ $errors->has('class_id') ? 'is-invalid' : '' }}"
                                ...
                                @foreach($commission_settings as $cs_key => $cs_item)
                                    <option value="{{ $cs_key }}"  @selected(old("class_id", $user->class_id) === $cs_key ) >{{ $cs_item }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('class'))
...
{{--                            <select class="form-control {{ $errors->has('city') ? 'is-invalid' : '' }} select2"--}}
{{--                                    name="city"--}}
{{--                                    id="city">--}}
...
{{--                                    <option value="{{ $id }}" {{ (old('bank_id') ? old('bank_id') : $user->bank->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>--}}
{{--                                @endforeach--}}
{{--                            </select>--}}
{{--                            @if($errors->has('bank'))--}}
{{--                                <div class="invalid-feedback">--}}
