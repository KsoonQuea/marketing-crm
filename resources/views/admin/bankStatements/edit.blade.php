...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.bankStatement.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.bank-statements.update", [$bankStatement->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bank') ? 'is-invalid' : '' }}" name="bank_id" id="bank_id">
                    @foreach($banks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('bank_id') ? old('bank_id') : $bankStatement->bank->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('bank'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('case') ? 'is-invalid' : '' }}" name="case_id" id="case_id">
                    @foreach($cases as $id => $entry)
                        <option value="{{ $id }}" {{ (old('case_id') ? old('case_id') : $bankStatement->case->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('case'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bank_owner') ? 'is-invalid' : '' }}" type="text" name="bank_owner" id="bank_owner" value="{{ old('bank_owner', $bankStatement->bank_owner) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bank_account') ? 'is-invalid' : '' }}" type="text" name="bank_account" id="bank_account" value="{{ old('bank_account', $bankStatement->bank_account) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('credit') ? 'is-invalid' : '' }}" type="number" name="credit" id="credit" value="{{ old('credit', $bankStatement->credit) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('debit') ? 'is-invalid' : '' }}" type="number" name="debit" id="debit" value="{{ old('debit', $bankStatement->debit) }}" step="0.01">
                @if($errors->has('debit'))
                    <div class="invalid-feedback">
                        {{ $errors->first('debit') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('month_end_balance') ? 'is-invalid' : '' }}" type="number" name="month_end_balance" id="month_end_balance" value="{{ old('month_end_balance', $bankStatement->month_end_balance) }}" step="0.01">
