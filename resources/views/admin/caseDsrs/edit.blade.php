...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.caseDsr.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.case-dsrs.update", [$caseDsr->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('case') ? 'is-invalid' : '' }}" name="case_id" id="case_id">
                    @foreach($cases as $id => $entry)
                        <option value="{{ $id }}" {{ (old('case_id') ? old('case_id') : $caseDsr->case->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('case'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ebitda') ? 'is-invalid' : '' }}" type="number" name="ebitda" id="ebitda" value="{{ old('ebitda', $caseDsr->ebitda) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ccris_commitment') ? 'is-invalid' : '' }}" type="number" name="ccris_commitment" id="ccris_commitment" value="{{ old('ccris_commitment', $caseDsr->ccris_commitment) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bank_statement_commitment') ? 'is-invalid' : '' }}" type="number" name="bank_statement_commitment" id="bank_statement_commitment" value="{{ old('bank_statement_commitment', $caseDsr->bank_statement_commitment) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('new_financing_commitment') ? 'is-invalid' : '' }}" type="number" name="new_financing_commitment" id="new_financing_commitment" value="{{ old('new_financing_commitment', $caseDsr->new_financing_commitment) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('dsr') ? 'is-invalid' : '' }}" type="number" name="dsr" id="dsr" value="{{ old('dsr', $caseDsr->dsr) }}" step="0.01">
                @if($errors->has('dsr'))
                    <div class="invalid-feedback">
                        {{ $errors->first('dsr') }}
                    </div>
