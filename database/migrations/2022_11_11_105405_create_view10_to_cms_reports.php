...
class CreateView10ToCmsReports extends Migration
{
    public function up()
    {
        \Illuminate\Support\Facades\DB::statement("
                ...
                SELECT cms_report.*,
                if(cms_report.or_date > CONCAT(cms_report.or_year_ori, '-', cms_report.or_month_ori, '-25'),
                if(cms_report.or_month_ori+1 = 13, cms_report.or_year_ori + 1, cms_report.or_year_ori), cms_report.or_year_ori) AS 'or_year',
                if(cms_report.or_date > CONCAT(cms_report.or_year_ori, '-', cms_report.or_month_ori, '-25'),
                if(cms_report.or_month_ori+1 = 13, 1, cms_report.or_month_ori+1), cms_report.or_month_ori) AS 'or_month'
                FROM (

                SELECT YEAR(payments.date) AS 'or_year_ori', MONTH(payments.date) AS 'or_month_ori',
    ...
    {
        Schema::dropIfExists('view10_to_cms_reports');
    }
}
